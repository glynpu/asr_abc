...
# Modified from https://github.com/k2-fsa/icefall for tutorial only.
#   1. remove unused modules by ctc decoding
#   2. demonstrate usage of debuging tools


# original paper of Conformer:
...
class Conformer(nn.Module):
    ...
    def forward(
        ...
        encoder_memory = self.run_encoder(x)
        x = self.ctc_output(encoder_memory)
        return x, encoder_memory

    def run_encoder(self, x: Tensor) -> Tuple[Tensor, Optional[Tensor]]:
        """
        Args:
          x:
        ...
        x = self.encoder(x, pos_emb)  # (T, B, F)
